...
//    $category_list[] = "tree"; add element to the category_list array
    $token = strtok($_GET['category_list'],"_");
    ...
    echo"t=".$t."<br>";
      ...
      $farthest_cards_sql = "SELECT * FROM admin_db ";   

      
      
      $query_rank5="SELECT * FROM admin_db WHERE rank='5' AND (";
      $query_rank4="SELECT * FROM admin_db WHERE rank='4' AND (";
      $query_rank3="SELECT * FROM admin_db WHERE rank='3' AND (";
      $query_rank2="SELECT * FROM admin_db WHERE rank='2' AND (";
      $query_rank1="SELECT * FROM admin_db WHERE rank='1' AND (";
      for($x=0;$x<count($category_list);$x++)
      {
         if($x==(count($category_list)-1))
         {
           $query_rank5.="`admin_db`.`category`='$category_list[$x]'";
           $query_rank4.="`admin_db`.`category`='$category_list[$x]'";
           $query_rank3.="`admin_db`.`category`='$category_list[$x]'";
           $query_rank2.="`admin_db`.`category`='$category_list[$x]'";
           $query_rank1.="`admin_db`.`category`='$category_list[$x]'";
           
         }
         else
         { 
           $query_rank5.="`admin_db`.`category`='$category_list[$x]' OR ";
           $query_rank4.="`admin_db`.`category`='$category_list[$x]' OR ";
           $query_rank3.="`admin_db`.`category`='$category_list[$x]' OR ";
           $query_rank2.="`admin_db`.`category`='$category_list[$x]' OR ";
           $query_rank1.="`admin_db`.`category`='$category_list[$x]' OR ";
         }
      }     
      $query_rank5.=" )";
      $query_rank4.=" )";
...
//        else if($rand_num <=60 && $rand_num>=41 && $rank3_num_rows!=0){$where_clause.="`admin_db`.`rank`=3 AND (";$card_rank=3; break;}
      ...
      for($x=0;$x<count($category_list);$x++)
      {
         if($x==(count($category_list)-1))$where_clause.="`admin_db`.`category`='$category_list[$x]'";
         else $where_clause.="`admin_db`.`category`='$category_list[$x]' OR ";
      }      
            
      $farthest_cards_sql=$farthest_cards_sql.$where_clause;
      $farthest_cards_sql=$farthest_cards_sql.") AND add_time=(SELECT MIN(add_time) FROM admin_db ";
      $farthest_cards_sql=$farthest_cards_sql.$where_clause.") )";
